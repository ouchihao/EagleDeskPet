...
先从右键菜单退出旧版，再运行已解压程序包里的 `EagleDeskPet.exe`。自包含发布包不需要另装 .NET；源码 ZIP 不能直接双击运行，需按 [README](../README.md) 构建。默认发布目录为 `dist/v1.10.1`。首次启动需要预载资源，稍等片刻。
...
右键“石头剪刀布 · 逗逗它…”打开 304×352 的可拖动小窗。小鹰先选好拳，你再选择图形拳型按钮；准备 0.6 秒，出拳 2.8 秒（清晰手势保持约 0.87 秒，再完整收手）后揭晓，输赢反应 2.4 秒，平局害羞仍为 2 秒。选拳后整套演出约 6 秒，不包含你选择拳型、首次预载和前一动作收尾的等待，也不是严格的总耗时保证。v1.10.1 的三套服装均使用新绘的 16 关键姿势；详见[动画修订](RPS-ANIMATION-v1.10.1.md)。
