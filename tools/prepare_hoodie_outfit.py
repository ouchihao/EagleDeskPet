...
def make_references(assets:Path):
    refs = assets/'AnimationSources/HoodieReferences'
...
def prepare_neutral(assets:Path):
    ...
    source = assets/'AnimationSources/hoodie-neutral-v1.png'
    ...
    target = assets/'Outfits/Hoodie'; target.mkdir(parents=True,exist_ok=True)
    ...
    for group, info in refs.items():
        ...
        for clip, indices in info['mapping'].items():
            ...
            if not key_qa['passed']:
                raise RuntimeError(f"Hoodie/{clip} keys failed QA: {key_qa['errors']}; {key_qa['preflight']['errors']}")
            print(json.dumps({'outfit':'hoodie','clip':clip,'keys_passed':True}),flush=True)
            ...
            print(json.dumps({'outfit':'hoodie','clip':clip,'passed':qa['passed']}),flush=True)
            if not qa['passed']: raise RuntimeError(f'Hoodie/{clip} failed QA; do not list for sale')

def audit(assets:Path):
    """Read every output and audit scene seams without modifying artwork."""
    target = assets/'Outfits/Hoodie'
    preview = assets/'AnimationPreviews/Hoodie'
    refs = load_mapping(assets)
    ...
    report={'passed':not errors,'errors':errors,'outfit':'outfit.hoodie','clips':clips,'scene_seams':seam_results,
    ...
    if errors: raise RuntimeError('Hoodie outfit audit failed; do not register for sale')
...
def transition_review(assets:Path,selected:list[str]|None=None):
    ...
    for clip,indices in samples.items():
        ...
        for column,index in enumerate(indices):
            frame=Image.open(assets/'Outfits/Hoodie/Animations'/clip/f'frame-{index:04d}.png').convert('RGBA')
            x=column*240
...
def make_manifest(assets:Path):
    """Publish this outfit manifest only after complete output/provenance validation.

    Office supplies the existing manifest schema, never fallback sprite frames.
    """
    audit(assets)
    catalog=json.loads((assets/'Outfits/Office/actions.json').read_text(encoding='utf-8'))
    expected={clip for clips in GROUPS.values() for clip in clips}
    actions=catalog.get('actions',[])
    directories=[Path(item['directory']).name for item in actions]
    if len(directories)!=19 or set(directories)!=expected:
        raise ValueError('Schema reference does not contain the expected 19 full outfit clips')
    catalog['neutral']='Assets/Outfits/Hoodie/neutral.png'
    for item in actions:
        clip=Path(item['directory']).name
        frames=sorted((assets/'Outfits/Hoodie/Animations'/clip).glob('frame-*.png'))
        seconds=TIMES.get(clip,(2,()))[0]
        if item['frameCount']!=len(frames) or item['durationSeconds']!=seconds:
            raise ValueError(f'{clip}: schema and generated timing disagree')
        item['directory']=f'Assets/Outfits/Hoodie/Animations/{clip}'
    destination=assets/'Outfits/Hoodie/actions.json'
    print(f'Exact outfit manifest output: {destination}',flush=True)
    destination.write_text(json.dumps(catalog,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')

def main():
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--assets',type=Path,required=True)
    p.add_argument('--phase',choices=['refs','build','audit','review','manifest'],required=True)
    ...
    elif args.phase=='manifest':make_manifest(assets)
