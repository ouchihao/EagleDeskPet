...
{
    var first = WorkStageMotion.Sample(At(ClipKind.WorkEnter, 0));
    var last = WorkStageMotion.Sample(At(ClipKind.WorkEnter, 1));
...
{
    var atZero = WorkStageMotion.Sample(AtSeconds(ClipKind.WorkEnter, 0));
    var duringSlide = WorkStageMotion.Sample(AtSeconds(ClipKind.WorkEnter, 0.6));
    var atDeskArrival = WorkStageMotion.Sample(AtSeconds(ClipKind.WorkEnter, 1.0));
    var atLanding = WorkStageMotion.Sample(AtSeconds(ClipKind.WorkEnter, 1.8));
    ...
    for (int i = 0; i <= 210; i++)
    {
        var motion = WorkStageMotion.Sample(At(ClipKind.WorkEnter, i / 210.0));
...
static void AnchoredLoops()
{
    foreach (var kind in new[] { ClipKind.WorkLoop, ClipKind.WorkToBusy, ClipKind.BusyLoop })
    {
        for (int i = 0; i <= 960; i++)
        {
            var motion = WorkStageMotion.Sample(At(kind, i / 960.0));
...
static void ExitEndpoints()
{
    ...
    {
        var first = WorkStageMotion.Sample(At(kind, 0));
        var last = WorkStageMotion.Sample(At(kind, 1));
        ...
        for (int i = 1; i <= 120; i++)
        {
            var motion = WorkStageMotion.Sample(At(kind, i / 120.0));
...
static void MotionBounds()
{
    foreach (var kind in Enum.GetValues<ClipKind>().Where(ClipCatalog.IsWorkScene))
    {
        ...
        var previous = WorkStageMotion.Sample(At(kind, 0));
...
static void NonWorkInvisible()
{
    foreach (var kind in Enum.GetValues<ClipKind>().Where(kind => !ClipCatalog.IsWorkScene(kind)))
    {
        ...
        Near(0, motion.DeskXPixels);
        Near(0, motion.LaptopYPixels);
        Near(0, WorkStageMotion.FireGrowth(clip));
    }
}

static void BusyGrowth()
{
    Near(0, WorkStageMotion.FireGrowth(At(ClipKind.WorkToBusy, 0)));
    Near(1, WorkStageMotion.FireGrowth(At(ClipKind.WorkToBusy, 1)));
    Near(0, WorkStageMotion.FireGrowth(AtSeconds(ClipKind.WorkToBusy, 0.25)));
    Near(1, WorkStageMotion.FireGrowth(AtSeconds(ClipKind.WorkToBusy, 1.3)));
    double previous = 0;
    for (int i = 0; i <= 360; i++)
    {
        double growth = WorkStageMotion.FireGrowth(At(ClipKind.WorkToBusy, i / 360.0));
        Check(growth >= previous - 1e-10, "Fire growth reversed during the busy bridge.");
        Check(growth - previous < 0.01, "Fire popped into view.");
        previous = growth;
    }
    for (int i = 0; i <= 120; i++) Near(1, WorkStageMotion.FireGrowth(At(ClipKind.BusyLoop, i / 120.0)));
}

static void ExitGrowth()
{
    Near(1, WorkStageMotion.FireGrowth(At(ClipKind.BusyExit, 0)));
    Near(0, WorkStageMotion.FireGrowth(At(ClipKind.BusyExit, 1)));
    Near(0, WorkStageMotion.FireGrowth(AtSeconds(ClipKind.BusyExit, 0.85)));
    double previous = 1;
    for (int i = 0; i <= 480; i++)
    {
        double growth = WorkStageMotion.FireGrowth(At(ClipKind.BusyExit, i / 480.0));
...
static void PhaseSeams()
{
    ...
    })
    {
        ...
        var endMotion = WorkStageMotion.Sample(end);
        var startMotion = WorkStageMotion.Sample(start);
        ...
        Near(WorkStageMotion.FireGrowth(end), WorkStageMotion.FireGrowth(start));
    }
}
