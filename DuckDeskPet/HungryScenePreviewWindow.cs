...
internal sealed class HungryScenePreviewWindow : Window
{
    ...
    internal HungryScenePreviewWindow(string outfitId, SceneSelection selection)
    {
        ...
        Title = "大头鹰 · 空碗小剧场"; Width = 420; Height = 510; MinWidth = 350; MinHeight = 390;
        Background = PreviewStageChrome.Paper; Foreground = PreviewStageChrome.Ink; FontFamily = new("Microsoft YaHei UI");
        ...
        var root = new Grid { Margin = new(17), Background = Background };
        root.RowDefinitions.Add(new() { Height = GridLength.Auto });
        root.RowDefinitions.Add(new()); root.RowDefinitions.Add(new() { Height = GridLength.Auto });
        var header = new StackPanel();
        header.Children.Add(PreviewStageChrome.Ticket("饭呢？我的饭呢？", "空碗小剧场 · 只看表演，不改变桌宠", "ACT 01"));
        ...
        var layers = new Grid { Width = 302, Height = 265, ClipToBounds = true, Background = Brushes.Transparent };
        foreach (var image in new[] { _fire, _back, _pet, _front, _computer }) layers.Children.Add(image);
        var viewport = PreviewStageChrome.Stage(layers);
        Grid.SetRow(viewport, 1); root.Children.Add(viewport);
        var footer = new StackPanel { Margin = new(0, 12, 0, 0) };
        _stageText = new() { Text = "正在准备完整的小剧场…", FontSize = 11, Foreground = PreviewStageChrome.Muted, TextWrapping = TextWrapping.Wrap };
        footer.Children.Add(_stageText);
        footer.Children.Add(new TextBlock { Text = "不扣粮、不花鹰币 · 演完后桌宠还是原样", FontSize = 10, Foreground = PreviewStageChrome.Muted, Margin = new(0, 5, 0, 10), TextWrapping = TextWrapping.Wrap });
        ...
        close.Style = (Style)FindResource("PetPrimaryButton");
